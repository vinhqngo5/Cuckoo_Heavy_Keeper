...
// note: file from https://github.dev/goossaert/kingdb, no need to read here
#pragma once
...
// TODO: have short letter version of parameter names? ex: -l or --loglevel for
// the same parameter
    ...
    std::string ToString() const {
        if (message1_ == "") {
            ...
            case kMultipartRequired:
                type = "MultipartRequired: the entry is too large to fit in "
                       "memory, use the multipart API instead.";
                break;
            default:
...
class Parameter {
  public:
    ...
    uint64_t GetMultiplier(std::string str) {
        ...
        std::smatch matches;
        if (!std::regex_search(str, matches, regex_number) ||
            matches.size() != 3) {
            return 1;
        }
        ...
        } else if (unit == "ms" || unit == "millisecond" ||
                   unit == "milliseconds") {
            return 1;
        ...
        if (value == "true" || value == "1") {
            ...
            *state = false;
            ...
            std::string str_line_number =
                "Invalid value for boolean parameter [" + config +
...
class UnsignedInt32Parameter : public Parameter {
  public:
    ...
    virtual Status Parse(const std::string &config, const std::string &value,
        ...
        if (num_scanned != 1) {
            std::string str_line_number =
                "Invalid value for unsigned 32-bit integer parameter [" +
                config + "] in file [" + filepath + "] on line " +
                std::to_string(line_number);
            return Status::IOError("ConfigParser", str_line_number);
        }
        ...
        if (multiplier == 0) {
            std::string str_line_number =
                "Invalid unit for parameter [" + config + "] in file [" +
                filepath + "] on line " + std::to_string(line_number);
            return Status::IOError("ConfigParser", str_line_number);
        }
        *ptr = *ptr * multiplier;
        return Status::OK();
...
class UnsignedInt64Parameter : public Parameter {
  public:
    ...
    virtual Status Parse(const std::string &config, const std::string &value,
        ...
        if (num_scanned != 1) {
            std::string str_line_number =
                "Invalid value for unsigned 64-bit integer parameter [" +
                config + "] in file [" + filepath + "] on line " +
                std::to_string(line_number);
            return Status::IOError("ConfigParser", str_line_number);
        }
...
class IntParameter : public Parameter {
  public:
    int *ptr;
    IntParameter(const std::string &name_in, const std::string default_in,
                 int *ptr_in, bool mandatory_in,
                 const std::string &description_in) {
        name = name_in;
        is_mandatory = mandatory_in;
        description = description_in;
    ...
    virtual int Get() { return *ptr; }
    virtual Status Parse(const std::string &config, const std::string &value,
                         const std::string &filepath, int line_number) {
        int num_scanned = sscanf(value.c_str(), "%u", ptr);
        if (num_scanned != 1) {
            std::string str_line_number =
                "Invalid value for 32-bit integer parameter [" + config +
                "] in file [" + filepath + "] on line " +
                std::to_string(line_number);
            return Status::IOError("ConfigParser", str_line_number);
        }
    ...
    virtual std::string Type() { return "32-bit integer"; }
};
...
class DoubleParameter : public Parameter {
  public:
    ...
    virtual Status Parse(const std::string &config, const std::string &value,
        ...
        if (num_scanned != 1) {
            std::string str_line_number =
                "Invalid value for double-precision number parameter [" +
                config + "] in file [" + filepath + "] on line " +
                std::to_string(line_number);
            return Status::IOError("ConfigParser", str_line_number);
        }
...
class FloatParameter : public Parameter {
  public:
    float *ptr;
    FloatParameter(const std::string &name_in, const std::string &default_in,
                   float *ptr_in, bool mandatory_in,
                   const std::string &description_in) {
        name = name_in;
        is_mandatory = mandatory_in;
        description = description_in;
    ...
    virtual float Get() { return *ptr; }
    virtual Status Parse(const std::string &config, const std::string &value,
                         const std::string &filepath, int line_number) {
        int num_scanned = sscanf(value.c_str(), "%f", ptr);
        if (num_scanned != 1) {
            std::string str_line_number =
                "Invalid value for float-precision number parameter [" +
                config + "] in file [" + filepath + "] on line " +
                std::to_string(line_number);
            return Status::IOError("ConfigParser", str_line_number);
        }
    ...
    virtual std::string Type() { return "Float-precision number"; }
};
...
class ConfigParser {
  public:
    ...
    void AddParameter(Parameter *parameter) {
        ...
        if (parameter->is_mandatory) { mandatories_.insert(parameter->name); }
    }
    ...
    void SetDefaultValue(const std::string name,
        ...
        if (it != parameters_.end()) { (*it)->default_value = default_value; }
    }
        ...
        for (auto &p : parameters_) {
            ...
            if (mandatories_.find(p->name) == mandatories_.end()) {
                fprintf(stdout, "%sDefault value: %s (%s)\n",
                        str_margin.c_str(), p->default_value.c_str(),
                        p->Type().c_str());
            } else {
    ...
    Status ParseCommandLine(int argc, char **argv) {
        ...
        for (auto &p : parameters_) { parameters[p->name] = p; }
        ...
        while (i < argc) {
            if (strncmp(argv[i], "--", 2) != 0) {
                if (error_if_unknown_parameters) {
                    std::string msg =
                        "Invalid parameter [" + std::string(argv[i]) + "]";
                    return Status::IOError("ConfigReader::ReadCommandLine()",
                                           msg);
                } else {
                    i++;
                    continue;
            ...
            bool is_flag_parameter = (dynamic_cast<FlagParameter *>(
                                          parameters[argument]) != nullptr);
            if (is_flag_parameter && has_equal_sign) {
                std::string msg =
                    "The argument [" + std::string(argv[i]) +
            ...
            if (!is_flag_parameter && !has_equal_sign) {
                if (i + 1 >= argc) {
                    std::string msg = "Missing value for parameter [" +
                                      std::string(argv[i]) + "]";
                    return Status::IOError("ConfigReader::ReadCommandLine()",
                                           msg);
                }
    ...
    Status ParseFile(std::string filepath) {
        ...
        if (stat(filepath.c_str(), &info) != 0) {
            return Status::IOError("ConfigParser",
        ...
        while (std::getline(file, str)) {
            ...
            *ptr = '\0';
            ...
            if (!std::regex_search(line_cleaned, matches, regex_config) ||
                matches.size() != 3) {
                std::string str_line_number = "Error in file [" + filepath +
                                              "] on line " +
                                              std::to_string(line_number);
                return Status::IOError("ConfigParser", str_line_number);
            }
            ...
            if (parameters.find(config) == parameters.end()) {
                if (error_if_unknown_parameters) {
                    std::string str_line_number =
                        "Unknown parameter [" + config + "] in file [" +
                        filepath + "] on line " + std::to_string(line_number);
                    return Status::IOError("ConfigParser", str_line_number);
                } else {
                    continue;
            ...
            Status s =
                parameters[config]->Parse(config, value, filepath, line_number);
