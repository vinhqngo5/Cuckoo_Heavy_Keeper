...
class cuckoohash_map {
  public:   // should be private
    ...
    using buckets_t = bucket_container<Key, T, Allocator, partial_t, SLOT_PER_BUCKET>;
  ...
  public:
    ...
    cuckoohash_map(size_type n = DEFAULT_SIZE, const Hash &hf = Hash(), const KeyEqual &equal = KeyEqual(), const Allocator &alloc = Allocator())
        : hash_fn_(hf), eq_fn_(equal), buckets_(reserve_calc(n), alloc), old_buckets_(0, alloc), all_locks_(get_allocator()), num_remaining_lazy_rehash_locks_(0),
          minimum_load_factor_(DEFAULT_MINIMUM_LOAD_FACTOR), maximum_hashpower_(NO_MAXIMUM_HASHPOWER), max_num_worker_threads_(0) {
    ...
    cuckoohash_map(const cuckoohash_map &other, const Allocator &alloc)
        : hash_fn_(other.hash_fn_), eq_fn_(other.eq_fn_), buckets_(other.buckets_, alloc), old_buckets_(other.old_buckets_, alloc), all_locks_(alloc),
          num_remaining_lazy_rehash_locks_(other.num_remaining_lazy_rehash_locks_), minimum_load_factor_(other.minimum_load_factor_), maximum_hashpower_(other.maximum_hashpower_),
    ...
    cuckoohash_map(cuckoohash_map &&other, const Allocator &alloc)
        : hash_fn_(std::move(other.hash_fn_)), eq_fn_(std::move(other.eq_fn_)), buckets_(std::move(other.buckets_), alloc), old_buckets_(std::move(other.old_buckets_), alloc),
          all_locks_(alloc), num_remaining_lazy_rehash_locks_(other.num_remaining_lazy_rehash_locks_), minimum_load_factor_(other.minimum_load_factor_),
          maximum_hashpower_(other.maximum_hashpower_), max_num_worker_threads_(other.max_num_worker_threads_) {
        if (other.get_allocator() == alloc) {
     ...
     * @param alloc allocator instance to use
     */
    cuckoohash_map(std::initializer_list<value_type> init, size_type n = DEFAULT_SIZE, const Hash &hf = Hash(), const KeyEqual &equal = KeyEqual(),
    ...
    void swap(cuckoohash_map &other) noexcept {
        ...
        other.minimum_load_factor_.store(minimum_load_factor_.exchange(other.minimum_load_factor(), std::memory_order_release), std::memory_order_release);
        other.maximum_hashpower_.store(maximum_hashpower_.exchange(other.maximum_hashpower(), std::memory_order_release), std::memory_order_release);
    }
    ...
    cuckoohash_map &operator=(std::initializer_list<value_type> ilist) {
        clear();
        for (const auto &item : ilist) { insert(item.first, item.second); }
        return *this;
    }
    ...
    size_type size() const {
        if (all_locks_.size() == 0) { return 0; }
        counter_type s = 0;
        for (spinlock &lock : get_current_locks()) { s += lock.elem_counter(); }
        assert(s >= 0);
     ...
     */
    double load_factor() const { return static_cast<double>(size()) / static_cast<double>(capacity()); }
     ...
     */
    double minimum_load_factor() const { return minimum_load_factor_.load(std::memory_order_acquire); }
    ...
    void maximum_hashpower(size_type mhp) {
        if (hashpower() > mhp) { throw std::invalid_argument("maximum hashpower " + std::to_string(mhp) + " is less than current hashpower"); }
     ...
     */
    size_type maximum_hashpower() const { return maximum_hashpower_.load(std::memory_order_acquire); }
     ...
     */
    void max_num_worker_threads(size_type extra_threads) { max_num_worker_threads_.store(extra_threads, std::memory_order_release); }

    /**
     * Returns the maximum number of extra worker threads.
     */
    size_type max_num_worker_threads() const { return max_num_worker_threads_.load(std::memory_order_acquire); }
    ...
    template <typename K, typename F> bool erase_fn(const K &key, F fn) {
        ...
        const table_position pos = cuckoo_find(key, hv.partial, b.i1, b.i2);
        if (pos.status == ok) {
            if (fn(buckets_[pos.index].mapped(pos.slot))) { del_from_bucket(pos.index, pos.slot); }
            return true;
        } else {
            return false;
        }
     ...
     * the table
     */
    template <typename K, typename F, typename... Args> bool uprase_fn(K &&key, F fn, Args &&...val) {
        ...
        if (pos.status == ok) {
            add_to_bucket(pos.index, pos.slot, hv.partial, std::forward<K>(key), std::forward<Args>(val)...);
            upsert_context = UpsertContext::NEWLY_INSERTED;
        } else {
            upsert_context = UpsertContext::ALREADY_EXISTED;
        }
        using CanInvokeWithUpsertContextT = typename internal::CanInvokeWithUpsertContext<F, mapped_type>::type;
        if (internal::InvokeUpraseFn(fn, buckets_[pos.index].mapped(pos.slot), upsert_context, CanInvokeWithUpsertContextT{})) { del_from_bucket(pos.index, pos.slot); }
     ...
     * operator()(mapped_type&)</tt>.
     */
    template <typename K, typename F, typename... Args> bool upsert(K &&key, F fn, Args &&...val) {
        constexpr bool kCanInvokeWithUpsertContext = internal::CanInvokeWithUpsertContext<F, mapped_type>::type::value;
        return uprase_fn(std::forward<K>(key), internal::UpsertToUpraseFn<F, mapped_type, kCanInvokeWithUpsertContext>{fn}, std::forward<Args>(val)...);
    }
    ...
    template <typename K, typename... Args> bool insert(K &&key, Args &&...val) {
        return upsert(std::forward<K>(key), [](mapped_type &) {}, std::forward<Args>(val)...);
    }
    ...
    template <typename K, typename V> bool insert_or_assign(K &&key, V &&val) {
        return upsert(std::forward<K>(key), [&val](mapped_type &m) { m = std::forward<V>(val); }, std::forward<V>(val));
    }
    ...
    locked_table lock_table_nonblocking() { return locked_table(*this, true); }
  ...
  public:   // should be private
    ...
    void add_locks_from_other(const cuckoohash_map &other) {
        ...
        std::copy(other_locks.begin(), other_locks.end(), get_current_locks().begin());
    }
    ...
    static constexpr bool is_simple() { return std::is_standard_layout<key_type>::value && std::is_trivial<key_type>::value && sizeof(key_type) <= 8; }

    // Whether or not the data is nothrow-move-constructible.
    static constexpr bool is_data_nothrow_move_constructible() {
        return std::is_nothrow_move_constructible<key_type>::value && std::is_nothrow_move_constructible<mapped_type>::value;
    }
    ...
    template <typename K> size_type hashed_key_only_hash(const K &key) const { return hash_function()(key); }
    ...
    static partial_t partial_key(const size_type hash) {
        ...
        const uint32_t hash_32bit = (static_cast<uint32_t>(hash_64bit) ^ static_cast<uint32_t>(hash_64bit >> 32));
        const uint16_t hash_16bit = (static_cast<uint16_t>(hash_32bit) ^ static_cast<uint16_t>(hash_32bit >> 16));
        const uint8_t hash_8bit = (static_cast<uint8_t>(hash_16bit) ^ static_cast<uint8_t>(hash_16bit >> 8));
        return hash_8bit;
    }

    // index_hash returns the first possible bucket that the given hashed key
    // could be.
    static inline size_type index_hash(const size_type hp, const size_type hv) { return hv & hashmask(hp); }
    ...
    static inline size_type alt_index(const size_type hp, const partial_t partial, const size_type index) {
    ...
    class LIBCUCKOO_ALIGNAS(64) spinlock {
      public:
        ...
        spinlock(const spinlock &other) noexcept : elem_counter_(other.elem_counter()), is_migrated_(other.is_migrated()) { lock_.clear(); }
        ...
        void lock() noexcept { while (lock_.test_and_set(std::memory_order_acq_rel)); }

        void unlock() noexcept { lock_.clear(std::memory_order_release); }

        bool try_lock() noexcept { return !lock_.test_and_set(std::memory_order_acq_rel); }
    ...
    template <typename U> using rebind_alloc = typename std::allocator_traits<allocator_type>::template rebind_alloc<U>;
    ...
    class TwoBuckets {
      public:
        TwoBuckets() {}
        TwoBuckets(size_type i1_, size_type i2_, locked_table_mode) : i1(i1_), i2(i2_) {}
        TwoBuckets(locks_t &locks, size_type i1_, size_type i2_, normal_mode)
            : i1(i1_), i2(i2_), first_manager_(&locks[lock_ind(i1)]), second_manager_((lock_ind(i1) != lock_ind(i2)) ? &locks[lock_ind(i2)] : nullptr) {}
    ...
    struct AllUnlocker {
        void operator()(cuckoohash_map *map) const {
            for (auto it = first_locked; it != map->all_locks_.end(); ++it) {
                locks_t &locks = *it;
                for (spinlock &lock : locks) { lock.unlock(); }
            }
        }
    ...
    template <bool IS_LAZY> void rehash_lock(size_t l) const noexcept {
        ...
        for (size_type bucket_ind = l; bucket_ind < old_buckets_.size(); bucket_ind += kMaxNumLocks) { move_bucket(old_buckets_, buckets_, bucket_ind); }
        lock.is_migrated() = true;

        if (IS_LAZY) { decrement_num_remaining_lazy_rehash_locks(); }
    }

    // locks the given bucket index.
    //
    // throws hashpower_changed if it changed after taking the lock.
    LockManager lock_one(size_type, size_type, locked_table_mode) const { return LockManager(); }
    ...
    //
    // throws hashpower_changed if it changed after taking the lock.
    TwoBuckets lock_two(size_type, size_type i1, size_type i2, locked_table_mode) const { return TwoBuckets(i1, i2, locked_table_mode()); }

    TwoBuckets lock_two(size_type hp, size_type i1, size_type i2, normal_mode) const {
    ...
    //
    // throws hashpower_changed if it changed after taking the lock.
    std::pair<TwoBuckets, LockManager> lock_three(size_type, size_type i1, size_type i2, size_type, locked_table_mode) const {
        return std::make_pair(TwoBuckets(i1, i2, locked_table_mode()), LockManager());
    }

    std::pair<TwoBuckets, LockManager> lock_three(size_type hp, size_type i1, size_type i2, size_type i3, normal_mode) const {
        ...
        if (l[1] != l[0]) { locks[l[1]].lock(); }
        if (l[2] != l[1]) { locks[l[2]].lock(); }
        ...
        return std::make_pair(TwoBuckets(locks, i1, i2, normal_mode()),
                              LockManager((lock_ind(i3) == lock_ind(i1) || lock_ind(i3) == lock_ind(i2)) ? nullptr : &locks[lock_ind(i3)]));
    }
    ...
    // hashpower.
    template <typename TABLE_MODE> TwoBuckets snapshot_and_lock_two(const hash_value &hv) const {
        while (true) {
    ...
    static inline size_type lock_ind(const size_type bucket_ind) { return bucket_ind & (kMaxNumLocks - 1); }
    ...
    template <typename K> table_position cuckoo_find(const K &key, const partial_t partial, const size_type i1, const size_type i2) const {
        int slot = try_read_from_bucket(buckets_[i1], partial, key);
        if (slot != -1) { return table_position{i1, static_cast<size_type>(slot), ok}; }
        slot = try_read_from_bucket(buckets_[i2], partial, key);
        if (slot != -1) { return table_position{i2, static_cast<size_type>(slot), ok}; }
    ...
    template <typename K> int try_read_from_bucket(const bucket &b, const partial_t partial, const K &key) const {
     ...
     */
    template <typename TABLE_MODE, typename K> table_position cuckoo_insert_loop(hash_value hv, TwoBuckets &b, K &key) {
        table_position pos;
        while (true) {
            const size_type hp = hashpower();
    ...
    template <typename TABLE_MODE, typename K> table_position cuckoo_insert(const hash_value hv, TwoBuckets &b, K &key) {
        int res1, res2;
        bucket &b1 = buckets_[b.i1];
        if (!try_find_insert_bucket(b1, res1, hv.partial, key)) { return table_position{b.i1, static_cast<size_type>(res1), failure_key_duplicated}; }
        bucket &b2 = buckets_[b.i2];
        if (!try_find_insert_bucket(b2, res2, hv.partial, key)) { return table_position{b.i2, static_cast<size_type>(res2), failure_key_duplicated}; }
        if (res1 != -1) { return table_position{b.i1, static_cast<size_type>(res1), ok}; }
        if (res2 != -1) { return table_position{b.i2, static_cast<size_type>(res2), ok}; }
        ...
        if (st == failure_under_expansion) {
            ...
            assert(TABLE_MODE() == locked_table_mode() || !get_current_locks()[lock_ind(b.i1)].try_lock());
            assert(TABLE_MODE() == locked_table_mode() || !get_current_locks()[lock_ind(b.i2)].try_lock());
            assert(!buckets_[insert_bucket].occupied(insert_slot));
            assert(insert_bucket == index_hash(hashpower(), hv.hash) || insert_bucket == alt_index(hashpower(), hv.partial, index_hash(hashpower(), hv.hash)));
    ...
    // for use afterwards.
    template <typename K, typename... Args> void add_to_bucket(const size_type bucket_ind, const size_type slot, const partial_t partial, K &&key, Args &&...val) {
        buckets_.setKV(bucket_ind, slot, partial, std::forward<K>(key), std::forward<Args>(val)...);
    ...
    template <typename K> bool try_find_insert_bucket(const bucket &b, int &slot, const partial_t partial, const K &key) const {
        // Silence a warning from MSVC about partial being unused if is_simple.
        (void) partial;
        slot = -1;
        for (int i = 0; i < static_cast<int>(slot_per_bucket()); ++i) {
            if (b.occupied(i)) {
                if (!is_simple() && partial != b.partial(i)) { continue; }
    ...
    // will not.
    template <typename TABLE_MODE> cuckoo_status run_cuckoo(TwoBuckets &b, size_type &insert_bucket, size_type &insert_slot) {
        ...
        try {
            while (!done) {
                const int depth = cuckoopath_search<TABLE_MODE>(hp, cuckoo_path, b.i1, b.i2);
                if (depth < 0) { break; }
    ...
    template <typename TABLE_MODE> int cuckoopath_search(const size_type hp, CuckooRecords &cuckoo_path, const size_type i1, const size_type i2) {
        ...
        for (int i = 1; i <= x.depth; ++i) {
            ...
            assert(prev.bucket == index_hash(hp, prev.hv.hash) || prev.bucket == alt_index(hp, prev.hv.partial, index_hash(hp, prev.hv.hash)));
    ...
    template <typename TABLE_MODE> bool cuckoopath_move(const size_type hp, CuckooRecords &cuckoo_path, size_type depth, TwoBuckets &b) {
        if (depth == 0) {
        ...
        while (depth > 0) {
            ...
            if (depth == 1) {
                ...
                std::tie(twob, extra_manager) = lock_three(hp, b.i1, b.i2, to.bucket, TABLE_MODE());
            } else {
            ...
            if (tb.occupied(ts) || !fb.occupied(fs) || hashed_key_only_hash(fb.key(fs)) != from.hv.hash) { return false; }

            buckets_.setKV(to.bucket, ts, fb.partial(fs), fb.movable_key(fs), std::move(fb.mapped(fs)));
    ...
    static constexpr size_type const_pow(size_type a, size_type b) { return (b == 0) ? 1 : a * const_pow(a, b - 1); }
    ...
    struct b_slot {
        ...
        uint16_t pathcode;
        static_assert(const_pow(slot_per_bucket(), MAX_BFS_PATH_LEN) < std::numeric_limits<decltype(pathcode)>::max(), "pathcode may not be large enough to encode a cuckoo "
                                                                                                                       "path");
        ...
        int8_t depth;
        static_assert(MAX_BFS_PATH_LEN - 1 <= std::numeric_limits<decltype(depth)>::max(), "The depth type must able to hold a value of"
                                                                                           " MAX_BFS_PATH_LEN - 1");
        static_assert(-1 >= std::numeric_limits<decltype(depth)>::min(), "The depth type must be able to hold a value of -1");
        b_slot() {}
        b_slot(const size_type b, const uint16_t p, const decltype(depth) d) : bucket(b), pathcode(p), depth(d) { assert(d < MAX_BFS_PATH_LEN); }
    };
    ...
    class b_queue {
      ...
      public:   // should be private
        ...
        // MAX_BFS_PATH_LEN.
        static_assert(slot_per_bucket() > 0, "SLOT_PER_BUCKET must be greater than 0.");
        static constexpr size_type MAX_CUCKOO_COUNT =
            2 * ((slot_per_bucket() == 1) ? MAX_BFS_PATH_LEN : (const_pow(slot_per_bucket(), MAX_BFS_PATH_LEN) - 1) / (slot_per_bucket() - 1));
    ...
    //
    // throws hashpower_changed if it changed during the search
    template <typename TABLE_MODE> b_slot slot_search(const size_type hp, const size_type i1, const size_type i2) {
        b_queue q;
        ...
        while (!q.empty()) {
            ...
            for (size_type i = 0; i < slot_per_bucket(); ++i) {
                ...
                if (x.depth < MAX_BFS_PATH_LEN - 1) {
                    assert(!q.full());
                    b_slot y(alt_index(hp, partial, x.bucket), x.pathcode * slot_per_bucket() + slot, x.depth + 1);
                    q.enqueue(y);
                }
            }
        }
    ...
    template <typename TABLE_MODE, typename AUTO_RESIZE> cuckoo_status cuckoo_fast_double(size_type current_hp) {
        ...
        // left to rehash, so this should be a no-op.
        {
            ...
            for (size_t i = 0; i < current_locks.size(); ++i) { rehash_lock<kIsNotLazy>(i); }
        ...
        if (old_buckets_.size() < kMaxNumLocks) {
            for (size_type i = 0; i < old_buckets_.size(); ++i) { move_bucket(old_buckets_, buckets_, i); }
            ...
            for (spinlock &lock : current_locks) { lock.is_migrated() = false; }
            num_remaining_lazy_rehash_locks(current_locks.size());
            if (std::is_same<TABLE_MODE, locked_table_mode>::value) { rehash_with_workers(); }
        }
        return ok;
    }

    void move_bucket(buckets_t &old_buckets, buckets_t &new_buckets, size_type old_bucket_ind) const noexcept {
        ...
        for (size_type old_bucket_slot = 0; old_bucket_slot < slot_per_bucket(); ++old_bucket_slot) {
            if (!old_bucket.occupied(old_bucket_slot)) { continue; }
            ...
            if ((old_bucket_ind == old_ihash && new_ihash == new_bucket_ind) || (old_bucket_ind == old_ahash && new_ahash == new_bucket_ind)) {
                ...
                assert((old_bucket_ind == old_ihash && new_ihash == old_ihash) || (old_bucket_ind == old_ahash && new_ahash == old_ahash));
                dst_bucket_ind = old_bucket_ind;
                dst_bucket_slot = old_bucket_slot;
            }
            new_buckets.setKV(dst_bucket_ind, dst_bucket_slot++, old_bucket.partial(old_bucket_slot), old_bucket.movable_key(old_bucket_slot),
    ...
    template <typename AUTO_RESIZE> cuckoo_status check_resize_validity(const size_type orig_hp, const size_type new_hp) {
        const size_type mhp = maximum_hashpower();
        if (mhp != NO_MAXIMUM_HASHPOWER && new_hp > mhp) { throw maximum_hashpower_exceeded(new_hp); }
        if (AUTO_RESIZE::value && load_factor() < minimum_load_factor()) { throw load_factor_too_low(minimum_load_factor()); }
    ...
    void maybe_resize_locks(size_type new_bucket_count) {
        locks_t &current_locks = get_current_locks();
        if (!(current_locks.size() < kMaxNumLocks && current_locks.size() < new_bucket_count)) { return; }
        ...
        for (spinlock &lock : new_locks) { lock.lock(); }
    ...
    template <typename TABLE_MODE, typename AUTO_RESIZE> cuckoo_status cuckoo_expand_simple(size_type new_hp) {
        auto all_locks_manager = lock_all(TABLE_MODE());
        const size_type hp = hashpower();
        ...
        cuckoohash_map new_map(hashsize(new_hp) * slot_per_bucket(), hash_function(), key_eq(), get_allocator());
        new_map.max_num_worker_threads(max_num_worker_threads());

        parallel_exec(0, hashsize(hp), [this, &new_map](size_type i, size_type end, std::exception_ptr &eptr) {
            try {
                for (; i < end; ++i) {
                    auto &bucket = buckets_[i];
                    for (size_type j = 0; j < slot_per_bucket(); ++j) {
                        if (bucket.occupied(j)) { new_map.insert(bucket.movable_key(j), std::move(bucket.mapped(j))); }
                    }
                }
    ...
    template <typename F> void parallel_exec_noexcept(size_type start, size_type end, F func) {
        ...
        std::vector<std::thread, rebind_alloc<std::thread>> threads(get_allocator());
    ...
    template <typename F> void parallel_exec(size_type start, size_type end, F func) {
        const size_type num_extra_threads = max_num_worker_threads();
        const size_type num_workers = 1 + num_extra_threads;
        size_type work_per_thread = (end - start) / num_workers;
        std::vector<std::thread, rebind_alloc<std::thread>> threads(get_allocator());
        ...
        std::vector<std::exception_ptr, rebind_alloc<std::exception_ptr>> eptrs(num_workers, nullptr, get_allocator());
        for (size_type i = 0; i < num_extra_threads; ++i) {
            threads.emplace_back(func, start, start + work_per_thread, std::ref(eptrs[i]));
            start += work_per_thread;
        }
    ...
    void rehash_with_workers() noexcept {
        locks_t &current_locks = get_current_locks();
        parallel_exec_noexcept(0, current_locks.size(), [this](size_type start, size_type end) {
            for (size_type i = start; i < end; ++i) { rehash_lock<kIsNotLazy>(i); }
        });
        num_remaining_lazy_rehash_locks(0);
    }
    ...
    size_type num_remaining_lazy_rehash_locks() const { return num_remaining_lazy_rehash_locks_.load(std::memory_order_acquire); }

    void num_remaining_lazy_rehash_locks(size_type n) const {
        num_remaining_lazy_rehash_locks_.store(n, std::memory_order_release);
        if (n == 0) { old_buckets_.clear_and_deallocate(); }
    }

    void decrement_num_remaining_lazy_rehash_locks() const {
        size_type old_num_remaining = num_remaining_lazy_rehash_locks_.fetch_sub(1, std::memory_order_acq_rel);
        assert(old_num_remaining >= 1);
        if (old_num_remaining == 1) { old_buckets_.clear_and_deallocate(); }
    }
    ...
    template <typename AtomicT> class CopyableAtomic : public std::atomic<AtomicT> {
      public:
        using std::atomic<AtomicT>::atomic;

        CopyableAtomic(const CopyableAtomic &other) noexcept : CopyableAtomic(other.load(std::memory_order_acquire)) {}
  ...
  public:
    ...
    class locked_table {
      public:
        ...
        class const_iterator {
          public:
            ...
            bool operator==(const const_iterator &it) const { return buckets_ == it.buckets_ && index_ == it.index_ && slot_ == it.slot_; }
            ...
            const_iterator &operator++() {
                ...
                for (; index_ < buckets_->size(); ++index_) {
                    for (; slot_ < slot_per_bucket(); ++slot_) {
                        if ((*buckets_)[index_].occupied(slot_)) { return *this; }
                    }
                    slot_ = 0;
                }
          ...
          protected:
            ...
            static std::pair<size_type, size_type> end_pos(const buckets_t &buckets) { return std::make_pair(buckets.size(), 0); }
            ...
            const_iterator(buckets_t &buckets, size_type index, size_type slot) noexcept : buckets_(std::addressof(buckets)), index_(index), slot_(slot) {
                if (std::make_pair(index_, slot_) != end_pos(*buckets_) && !(*buckets_)[index_].occupied(slot_)) { operator++(); }
            }
        ...
        class iterator : public const_iterator {
          public:
            ...
            bool operator==(const iterator &it) const { return const_iterator::operator==(it); }

            bool operator!=(const iterator &it) const { return const_iterator::operator!=(it); }

            reference operator*() { return (*const_iterator::buckets_)[const_iterator::index_].kvpair(const_iterator::slot_); }
          ...
          public:   // should be private
            iterator(buckets_t &buckets, size_type index, size_type slot) noexcept : const_iterator(buckets, index, slot) {}
        ...
        static constexpr size_type slot_per_bucket() { return cuckoohash_map::slot_per_bucket(); }
        ...
        locked_table(locked_table &&lt) noexcept : map_(std::move(lt.map_)), all_locks_manager_(std::move(lt.all_locks_manager_)) {}
        ...
        void minimum_load_factor(const double mlf) { map_.get().minimum_load_factor(mlf); }

        double minimum_load_factor() const { return map_.get().minimum_load_factor(); }

        void maximum_hashpower(size_type mhp) { map_.get().maximum_hashpower(mhp); }

        size_type maximum_hashpower() const { return map_.get().maximum_hashpower(); }

        void max_num_worker_threads(size_type extra_threads) { map_.get().max_num_worker_threads(extra_threads); }

        size_type max_num_worker_threads() const { return map_.get().max_num_worker_threads(); }
        ...
        const_iterator begin() const { return const_iterator(map_.get().buckets_, 0, 0); }
        ...
        iterator end() {
            ...
            return iterator(map_.get().buckets_, static_cast<size_type>(end_pos.first), static_cast<size_type>(end_pos.second));
        }

        const_iterator end() const {
            const auto end_pos = const_iterator::end_pos(map_.get().buckets_);
            return const_iterator(map_.get().buckets_, static_cast<size_type>(end_pos.first), static_cast<size_type>(end_pos.second));
        }
         ...
         */
        template <typename K, typename... Args> std::pair<iterator, bool> insert(K &&key, Args &&...val) {
            hash_value hv = map_.get().hashed_key(key);
            auto b = map_.get().template snapshot_and_lock_two<locked_table_mode>(hv);
            table_position pos = map_.get().template cuckoo_insert_loop<locked_table_mode>(hv, b, key);
            if (pos.status == ok) {
                map_.get().add_to_bucket(pos.index, pos.slot, hv.partial, std::forward<K>(key), std::forward<Args>(val)...);
            } else {
                assert(pos.status == failure_key_duplicated);
            }
            return std::make_pair(iterator(map_.get().buckets_, pos.index, pos.slot), pos.status == ok);
        }
        ...
        template <typename K> size_type erase(const K &key) {
            ...
            const auto b = map_.get().template snapshot_and_lock_two<locked_table_mode>(hv);
            const table_position pos = map_.get().cuckoo_find(key, hv.partial, b.i1, b.i2);
            if (pos.status == ok) {
        ...
        template <typename K> size_type count(const K &key) const {
            const hash_value hv = map_.get().hashed_key(key);
            const auto b = map_.get().template snapshot_and_lock_two<locked_table_mode>(hv);
            return map_.get().cuckoo_find(key, hv.partial, b.i1, b.i2).status == ok ? 1 : 0;
        }
         ...
         */
        void rehash(size_type n) { map_.get().template cuckoo_rehash<locked_table_mode>(n); }

        /**
         * This has the same behavior as @ref cuckoohash_map::reserve, except
         * that we don't return anything.
         */
        void reserve(size_type n) { map_.get().template cuckoo_reserve<locked_table_mode>(n); }
        ...
        bool operator==(const locked_table &lt) const {
            if (size() != lt.size()) { return false; }
            for (const auto &elem : lt) {
                auto it = find(elem.first);
                if (it == end() || it->second != elem.second) { return false; }
            }
            return true;
        }

        bool operator!=(const locked_table &lt) const {
            if (size() != lt.size()) { return true; }
            for (const auto &elem : lt) {
                auto it = find(elem.first);
                if (it == end() || it->second != elem.second) { return true; }
            }
            return false;
        }
      ...
      public:   // should be private
        ...
        locked_table(cuckoohash_map &map) noexcept : map_(map), all_locks_manager_(map.lock_all(normal_mode())) { map.rehash_with_workers(); }
        locked_table(cuckoohash_map &map, bool non_blocking) noexcept : map_(map) {}
        ...
        void maybe_resize_locks(size_type new_bucket_count) { map_.get().maybe_resize_locks(new_bucket_count); }
        ...
        friend std::istream &operator>>(std::istream &is, locked_table &lt) {
            ...
            for (auto &lock : lt.get_current_locks()) { lock.elem_counter() = 0; }
            size_type size;
            is.read(reinterpret_cast<char *>(&size), sizeof(size_type));
            if (size > 0) { lt.get_current_locks()[0].elem_counter() = size; }
