...
class EMFSD {
  ...
  private:
    ...
    struct BetaGenerator {
        ...
        bool get_new_comb() {
            for (int j = now_flow_num - 2; j >= 0; --j) {
                int t = ++now_result[j];
                for (int k = j + 1; k < now_flow_num - 1; ++k) { now_result[k] = t; }
                int partial_sum = 0;
                for (int k = 0; k < now_flow_num - 1; ++k) { partial_sum += now_result[k]; }
            ...
            while (now_flow_num < flow_num_limit) {
                ...
                default:
                    ...
                    if (get_new_comb()) {
                        ...
                        now_flow_num++;
                        for (int i = 0; i < now_flow_num - 2; ++i) { now_result[i] = 1; }
    ...
    double get_p_from_beta(BetaGenerator &bt, double lambda, vector<double> &now_dist,
                           double now_n) {
        std::unordered_map<uint32_t, uint32_t> mp;
        for (int i = 0; i < bt.now_flow_num; ++i) { mp[bt.now_result[i]]++; }
    ...
    int collect_counters(uint32_t *counters) {
        ...
        for (uint32_t i = 0; i < w; ++i) { counter_dist[counters[i]]++; }
        /*
                std::cout << w << endl;
